...
router.post('/v1/chat/completions', authenticateApiKey, async (req, res) => {
  ...
  try {
    ...
    logger.info(`🔍 OpenAI v1/chat/completions API Key data:`, {
    ...
    if (
      ...
      logger.info(
        `🔒 OpenAI v1/chat/completions Model restriction check - Requested model: ${requestedModel}, Restricted models: ${JSON.stringify(apiKeyData.restrictedModels)}`
      )

      if (requestedModel && apiKeyData.restrictedModels.includes(requestedModel)) {
        logger.warn(
          `🚫 OpenAI v1/chat/completions Model restriction violation for key ${apiKeyData.name}: Attempted to use restricted model ${requestedModel}`
        )
        return res.status(403).json({
          error: {
            type: 'forbidden',
    ...
    upstream = await axios.post('https://api.openai.com/v1/chat/completions', req.body, axiosConfig)
    ...
    logger.error('OpenAI v1/chat/completions proxy failed:', error)
